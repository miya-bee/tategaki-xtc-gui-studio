...
CENTER_SETTINGS_BEHAVIOR_SECTION_KEY = 'behavior'
# These object names deliberately keep the old left-settings strings because
# QSS/tests may still target them.  The public helper names now describe the
# current v1.3.8 center settings pane role.
CENTER_SETTINGS_LEGACY_CONTAINER_OBJECT_NAME = 'leftSettingsContainer'
CENTER_SETTINGS_LEGACY_BOTTOM_SEPARATOR_OBJECT_NAME = 'leftSettingsBottomSep'
...
def build_center_settings_container_plan() -> dict[str, Any]:
    """Return the v1.3.8 center-settings pane layout plan.

    Object names intentionally keep the older left-settings strings for QSS and
    test compatibility.  The helper name reflects the current three-pane role.
    """
    return {
        'container_object_name': CENTER_SETTINGS_LEGACY_CONTAINER_OBJECT_NAME,
        ...
        'scroll_widget_resizable': False,
        'scroll_frame_shape': 'no_frame',
        'scroll_horizontal_scroll_bar_policy': 'always_on',
        'scroll_vertical_scroll_bar_policy': 'always_on',
        'scroll_minimum_content_width': 640,
        'bottom_separator_frame_shape': 'hline',
        'bottom_separator_object_name': CENTER_SETTINGS_LEGACY_BOTTOM_SEPARATOR_OBJECT_NAME,
        'bottom_separator_height': 1,
        'bottom_panel_min_height': 120,
    }


def build_left_settings_container_plan() -> dict[str, Any]:
    """Compatibility wrapper for the pre-v1.3.8 settings-pane name."""
    return build_center_settings_container_plan()


def build_center_settings_section_keys(*, include_behavior: object = False) -> tuple[str, ...]:
    """Return section keys for the v1.3.8 center settings pane."""
    if _coerce_bool_value(include_behavior):
        return CENTER_SETTINGS_SECTION_KEYS + (CENTER_SETTINGS_BEHAVIOR_SECTION_KEY,)
    return CENTER_SETTINGS_SECTION_KEYS


def build_left_settings_section_keys(*, include_behavior: object = False) -> tuple[str, ...]:
    """Compatibility wrapper for the pre-v1.3.8 settings-pane name."""
    return build_center_settings_section_keys(include_behavior=include_behavior)
...
def build_center_settings_section_layout_plan(section_key: object) -> dict[str, Any]:
    """Return layout defaults for a v1.3.8 center settings section."""
...
def build_left_settings_section_layout_plan(section_key: object) -> dict[str, Any]:
    """Compatibility wrapper for the pre-v1.3.8 settings-section name."""
    return build_center_settings_section_layout_plan(section_key)
...
def build_file_viewer_section_plan() -> dict[str, Any]:
    return {
        ...
        'open_xtc_help_text': '既存の .xtc / .xtch ファイルを右ペインへ読み込んで確認します。',
    }


def build_image_section_plan() -> dict[str, Any]:
    return {
        'ruby_hide_label': 'ルビ消し',
        'ruby_hide_text': 'ルビを表示しない',
        'ruby_hide_checked_default': False,
        'night_mode_text': '白黒反転',
        ...
        'help_text': 'ルビ消し: チェックした場合だけ、親文字は残したままルビを表示しない変換モードにします。\n白黒反転: 白と黒を入れ替えて出力します。プレビューにも反映されます。\nディザリング: 粒状感と引き換えに濃淡感を残します。\nしきい値: 白と黒の分かれ目を調整します。',
        'trailing_stretch': True,
    }


def build_preset_side_panel_plan() -> dict[str, Any]:
    return {
        'container_object_name': 'presetSideContainer',
        'contents_margins': (10, 9, 10, 9),
        'spacing': 8,
        'minimum_content_width': 240,
        'scroll_frame_shape': 'no_frame',
        'scroll_horizontal_scroll_bar_policy': 'as_needed',
        'scroll_vertical_scroll_bar_policy': 'as_needed',
    }
...
def build_preset_section_plan(*, minimum_button_width: object = 104) -> dict[str, Any]:
    return {
        'row_spacing': 8,
        'apply_button_text': 'プリセット\n読み込み',
        'apply_tooltip': '選択中のプリセットを現在の組版へ読み込みます',
        'save_button_text': 'プリセット\n保存',
        'save_tooltip': '現在の組版設定をこのプリセットへ保存します',
        'rename_button_text': '名称変更',
        'rename_tooltip': '選択中のプリセットの表示名を変更します',
...
def build_top_bar_plan(*, path_button_width: object = 128) -> dict[str, Any]:
    return {
        ...
        'target_tooltip': '変換対象のファイルまたはフォルダを入力します。ソースファイルはここへドラッグ＆ドロップできます。',
...
def build_view_toggle_bar_plan() -> dict[str, Any]:
    return {
        ...
        'right_pane_text': '右ペイン',
        'device_view_text': '右ペイン',
...
def build_right_preview_panel_plan() -> dict[str, Any]:
    return {
        ...
        'font_scroll_horizontal_scroll_bar_policy': 'always_on',
        'font_scroll_vertical_scroll_bar_policy': 'as_needed',
        ...
        'device_scroll_horizontal_scroll_bar_policy': 'always_on',
        'device_scroll_vertical_scroll_bar_policy': 'as_needed',
...
def build_bottom_panel_layout_plan() -> dict[str, Any]:
    return {
        ...
        'external_scrollbar_enabled': False,
...
def build_log_tab_plan(*, log_path: object = '') -> dict[str, Any]:
    return {
        ...
        'log_edit_vertical_scroll_bar_policy': 'always_on',
        'log_edit_horizontal_scroll_bar_policy': 'as_needed',
...
def build_results_tab_plan(*, summary_text: object = '変換結果の概要をここに表示します。') -> dict[str, Any]:
    return {
        ...
        'summary_label_object_name': 'resultsPlaceholderLabel',
        'summary_label_word_wrap': True,
        'summary_label_alignment': 'center',
        'summary_scroll_widget_resizable': True,
        'summary_scroll_frame_shape': 'no_frame',
        'summary_scroll_horizontal_scroll_bar_policy': 'as_needed',
        'summary_scroll_vertical_scroll_bar_policy': 'always_on',
        'results_list_vertical_scroll_bar_policy': 'always_on',
        'results_list_horizontal_scroll_bar_policy': 'as_needed',
...
def build_margin_rows_plan(
    *,
    ...
    return {
        ...
        # UI request: show the four margin controls on a single row.
        # Final requested left-to-right widget order:
        # 「上余白・下余白・左余白・右余白」.
        'labels': ('上余白', '下余白', '左余白', '右余白'),
        'trailing_stretch': True,
    }
