...
tar -xf tategaki-xtc-gui-studio_v1.3.2-release.zip

cd /d C:\Users\%USERNAME%\Downloads\tategaki-xtc-gui-studio_v1.3.2-release
```
...
環境によって venv 内の `python.exe` が Windows 側で実行できない場合があります。その場合は、Python Launcher の `py -3.10` を使って起動・テストできます。

```cmd
py -3.10 -B tategakiXTC_gui_studio.py
```
