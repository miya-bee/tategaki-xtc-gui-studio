...
v1.3.3 は、v1.3.2 公開後の hotfix 版です。README の公開版説明の整合性を修正し、CJK 互換縦書き閉じ括弧 `﹂` / `﹄` を直接入力した場合にも、下鍵括弧の5モード補正が効くようにしました。

v1.3.2 は、v1.3.1 を基準に、下鍵括弧の位置補正を句読点などと同じ5モードへ拡張し、プレビュー枚数の初期値を20ページに戻した小改良版です。

v1.3.0 は、v1.2.2 を基準に、フォルダ一括変換を中心として、複数ファイルをまとめて XTC / XTCH へ変換する導線を追加した安定版です。
...
- 前回公開版: `v1.3.2`
- 今回公開版: `v1.3.3`
- GitHub Release tag: `v1.3.3`
- GitHub Release title: `v1.3.3`

## v1.3.3 の主な更新

- README の v1.3.2 / v1.3.1 更新内容と前回公開版表記の不一致を修正しました。
- CJK 互換縦書き閉じ括弧 `﹂` / `﹄` を直接入力した場合にも、下鍵括弧の5モード補正が適用されるようにしました。
- `﹂` / `﹄` 直接入力時の下補正・上補正を確認する回帰テストを追加しました。

## v1.3.2 の主な更新

- 下鍵括弧の補正モードを、句読点・漢数字一と同じ `下補正 強 / 下補正 弱 / 標準 / 上補正 弱 / 上補正 強` の5モードに拡張しました。
...
- 失敗ファイルがある場合の完了ダイアログとログ表示を整理し、ログ末尾に `[ERROR-SUMMARY]` を出すようにしました。
- EPUB optional dependency 不足時の案内を改善し、`install_requirements.bat` / `requirements.txt` への導線を追加しました。
- フォルダ一括変換中の進捗表示を `1/N 件目｜ファイル名` 形式に整理しました。
- 停止ボタン押下後の文言と停止完了時の表示を整理し、未処理件数を確認できるようにしました。
- 上ペインのボタン名を「ファイルを開く...」「保存先を選ぶ...」「フォルダ一括変換...」に整理し、違いを説明する `?` ヘルプを追加しました。

## v1.3.0 の主な更新

- 上ペインの「フォルダ一括変換...」から、フォルダ内の複数ファイルをまとめて変換できます。
- サブフォルダ内の対象ファイルも処理できます。
- サブフォルダ構造を保持した出力に対応しました。
- 既存ファイルがある場合の扱いとして、スキップ、別名保存、上書きを選べます。
- 変換完了時に、成功・スキップ・失敗・処理済み件数・出力先・スキップ内訳を表示します。
- EPUB 変換に必要な optional dependency が不足している場合は、変換開始前に不足ライブラリとインストール例を案内します。
- PNG など画像由来のフォルダ一括変換結果を、正しい XTC / XTCH コンテナとして保存するようにしました。
...
- `RELEASE_NOTES_v1_3_3.md` — v1.3.3 の更新内容
- `RELEASE_NOTES_v1_3_2.md` — v1.3.2 の更新内容
- `RELEASE_NOTES_v1_3_1.md` — v1.3.1 の更新内容
...
半角コンマ `,` や半角ピリオド `.` は、フォントによって縦書き上で左下寄りに見えることがあります。青空文庫系の小説本文では大きな問題になりにくいため、v1.3.3 でも仕様として扱っています。
...
縦書き表示では、フォントによって記号の位置や形が大きく変わることがあります。v1.3.3 では、句読点、漢数字一、下鍵括弧、波線描画、波線位置をGUIから調整できます。下鍵括弧も、句読点などと同じ5モードで上下補正できます。迷った場合は、まず標準設定のまま使い、気になる記号だけ補正してください。
...
一部の統合ゲートテストはプロジェクトルート上のファイル構成を検証します。v1.3.3 でも主要な静的統合テストはテストファイル位置からプロジェクトルートを解決するため、別ディレクトリから個別実行しても参照先がずれにくくなっています。
...
作成済み release zip の検証は以下です。
    ...
    .venv\Scripts\python.exe -B ^
      build_release_zip.py ^
      --verify ^
      dist\tategaki-xtc-gui-studio_v1.3.3-release.zip
...
検証は任意の zip パスを指定できます。
    ...
    python build_release_zip.py --verify dist\tategaki-xtc-gui-studio_v1.3.3-release.zip

v1.3.3 の release zip は Python GUI版の source 構成に加えて `Font/` を同梱できます。Font フォルダが同梱されている release zip では、別コピー手順は不要です。source-only 配布に切り替える場合は、対応するフォント本体と `LICENSE_OFL.txt` の扱いを合わせてください。

## GitHub Release での公開方針

v1.3.3 は、GitHub 上では以下の扱いで公開します。
...
Release 本文には `RELEASE_NOTES_v1_3_3.md` の内容を使用します。

開発用の `.venv/` や `node_modules/` は release 対象外です。

## 公開対象について

v1.3.3 の公開対象は **Python GUI版のみ**です。
