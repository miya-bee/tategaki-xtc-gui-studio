...
半角コンマ `,` や半角ピリオド `.` は、フォントによって縦書き上で左下寄りに見えることがあります。青空文庫系の小説本文では大きな問題になりにくいため、v1.3.2 でも仕様として扱っています。
...
縦書き表示では、フォントによって記号の位置や形が大きく変わることがあります。v1.3.2 では、句読点、漢数字一、下鍵括弧、波線描画、波線位置をGUIから調整できます。下鍵括弧も、句読点などと同じ5モードで上下補正できます。迷った場合は、まず標準設定のまま使い、気になる記号だけ補正してください。
...
一部の統合ゲートテストはプロジェクトルート上のファイル構成を検証します。v1.3.2 でも主要な静的統合テストはテストファイル位置からプロジェクトルートを解決するため、別ディレクトリから個別実行しても参照先がずれにくくなっています。
...
作成済み release zip の検証は以下です。
    ...
    .venv\Scripts\python.exe -B ^
      build_release_zip.py ^
      --verify ^
      dist\tategaki-xtc-gui-studio_v1.3.2-release.zip
...
検証は任意の zip パスを指定できます。
    ...
    python build_release_zip.py --verify dist\tategaki-xtc-gui-studio_v1.3.2-release.zip

v1.3.2 の release zip は Python GUI版の source 構成に加えて `Font/` を同梱できます。Font フォルダが同梱されている release zip では、別コピー手順は不要です。source-only 配布に切り替える場合は、対応するフォント本体と `LICENSE_OFL.txt` の扱いを合わせてください。

## GitHub Release での公開方針

v1.3.2 は、GitHub 上では以下の扱いで公開します。
...
Release 本文には `RELEASE_NOTES_v1_3_2.md` の内容を使用します。

開発用の `.venv/` や `node_modules/` は release 対象外です。

## 公開対象について

v1.3.2 の公開対象は **Python GUI版のみ**です。
