...
迷った場合は標準設定のまま使い、気になる記号だけ補正してください。
...
同じ名前の出力ファイルが既に存在する場合の扱いは、右上の歯車メニュー内「その他オプション > 同名出力」から設定できます。

必要に応じて、既存ファイルを上書きするか、別名で保存するか、変換前に確認するかを選択してください。
...
このリポジトリには、Windowsでの起動・セットアップ・テストを補助するバッチファイルを同梱しています。

- `install_requirements.bat` — 依存ライブラリをインストールします
- `run_gui.bat` — GUIを起動します
- `run_tests.bat` — 代表的なテストを実行します

これらのバッチファイルは、展開したアプリフォルダー内で実行することを想定しています。

## テストと coverage の運用

GitHub Actions では、Python 3.10 / 3.11 を対象に代表的な回帰テストを実行します。

coverage は参考指標として扱い、単純な行カバレッジ率だけで品質を判断しない方針です。  
描画差分、release zip の内容、ドキュメント整合性、Windows 実機での操作確認をあわせて確認します。

README や release notes などの公開文書も、回帰テストの対象です。  
公開版の説明、配布物の境界、フォントの扱い、既知の制限が古くならないように確認しています。
